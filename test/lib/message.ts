...
describe('Message', function () {
    describe('constructor', function () {
        ...
        it('should also support a char array', function () {
            ...
            assert.deepStrictEqual(
                new Message(msg.toCharArray()).toCharArray(),
                msg.toCharArray(),
            );
        });
    });
    describe('static string2chars()', function () {
        ...
        it('should handle german umlauts', function () {
            assert.deepStrictEqual(
                Message.string2chars('Käse'),
                [11, 1, 5, 19, 5],
            );
        });
        it('should handle color emojis', function () {
            assert.deepStrictEqual(
                Message.string2chars('🟥🟧🟨🟩🟦🟪⬜⬛'),
                [63, 64, 65, 66, 67, 68, 69, 0],
            );
        });
        it('should remove special chars', function () {
            assert.deepStrictEqual(
                Message.string2chars('H⍷llo', { fallbackChar: null }),
                [8, 12, 12, 15],
            );
            assert.deepStrictEqual(
                Message.string2chars('H⍷llo'),
                [8, 60, 12, 12, 15],
            );
        });
        it('should work with all charts defined in CHAR_MAP', function () {
            ...
            CHAR_MAP.forEach(([str, chars]) => {
                ...
                const actual = Message.string2chars(str, {
                    removeUnsupportedWords: false,
                });
                const expected =
                    chars.length !== 1 || chars[0] !== -2 ? chars : [0];
                assert.deepStrictEqual(
                    actual,
                    expected,
                    `Unable to process char "${str}": string2chars returned ${JSON.stringify(actual)}, but expected result was ${JSON.stringify(expected)}`,
                );
            ...
            assert.deepStrictEqual(
                Message.string2chars(text, { removeUnsupportedWords: false }),
                result,
            );
        });
    });
    describe('static splitCharsIntoLines()', function () {
        ...
        it('should auto-split very long words', function () {
            const chars = Message.string2chars(
                'Rindfleischetikettierungsueberwachungsaufgabenuebertragungsgesetz',
            );
            assert.deepEqual(Message.splitCharsIntoLines(chars, [22, 22]), [
                [
                    18, 9, 14, 4, 6, 12, 5, 9, 19, 3, 8, 5, 20, 9, 11, 5, 20,
                    20, 9, 5, 18, 44,
                ],
                [
                    21, 14, 7, 19, 21, 5, 2, 5, 18, 23, 1, 3, 8, 21, 14, 7, 19,
                    1, 21, 6, 7, 44,
                ],
                [
                    1, 2, 5, 14, 21, 5, 2, 5, 18, 20, 18, 1, 7, 21, 14, 7, 19,
                    7, 5, 19, 5, 44,
                ],
                [20, 26],
            ]);
    ...
    describe('static removeEmojisFromChars()', function () {
        ...
        it('should trim emoji prefixes', function () {
            assert.deepStrictEqual(
                Message.removeEmojisFromChars(Message.string2chars('👋🏼Hello')),
                [8, 5, 12, 12, 15],
            );
            assert.deepStrictEqual(
                Message.removeEmojisFromChars(Message.string2chars('👋🏼 Hello')),
    ...
    describe('static getColumnSizesFromData()', function () {
        it('should work with a very basic example', function () {
            assert.deepStrictEqual(
                Message.getColumnSizesFromData([
                    ['now', 'Daily'],
        ...
        it('should work with only one column', function () {
            assert.deepStrictEqual(
                Message.getColumnSizesFromData([['Daily'], ['Secret Meeting']]),
                [22],
            );
        });
    ...
    describe('write()', function () {
        ...
        it('should support the NO_SPACE_BETWEEN flag', function () {
            ...
            msg.write('Rindfleisch')
                .write('etikettierungs', {
                    position: MessageWritePosition.NO_SPACE_BETWEEN,
                })
                .write('ueberwachungs', {
                    position: MessageWritePosition.NO_SPACE_BETWEEN,
                })
        ...
        it('should handle the fallback char', function () {
            assert.strictEqual(
                new Message().write('H⍷llo World').toString(),
                '#==============================================#\n' +
                    '# H ? L L O   W O R L D                        #\n' +
                    '#                                              #\n' +
                    '#                                              #\n' +
