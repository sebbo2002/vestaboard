    ...
    describe('getViewer()', function () {
        it('should work (mocked)', async function () {
            ...
            const fetch: RequestOptionsFetch = async (url, options) => {
                assert.strictEqual(
                    url,
                    'https://platform.vestaboard.com/viewer',
                );
        ...
        it(
            ...
            RUN_INTEGRATION_TESTS
                ? async function () {
                      ...
                      assert.strictEqual(
                          typeof result.installation._id,
                          'string',
                      );
                  }
                : undefined,
            ...
            RUN_INTEGRATION_TESTS
                ? async function () {
                      ...
                      result.subscriptions.forEach((subscription) => {
                          ...
                          assert.strictEqual(
                              typeof subscription._created,
                              'string',
                          );
                          assert.strictEqual(
                              typeof subscription.installation,
                              'object',
                          );
                          assert.strictEqual(
                              typeof subscription.installation._id,
                              'string',
                          );
                          assert.strictEqual(
                              typeof subscription.installation.installable,
                              'object',
                          );
                          assert.strictEqual(
                              typeof subscription.installation.installable?._id,
                              'string',
                          );

                          assert.strictEqual(
                              Array.isArray(subscription.boards),
                              true,
                          );
    ...
    describe('postMessage()', function () {
        it('should work (single mode)', async function () {
            ...
            const fetch: RequestOptionsFetch = async (url, options) => {
                assert.strictEqual(
                    url,
                    'https://platform.vestaboard.com/subscriptions/foo/message',
                );
                assert.deepStrictEqual(options, {
            ...
            RUN_INTEGRATION_TESTS
                ? async function () {
                      ...
                      result.forEach((message) => {
                          ...
                          assert.strictEqual(
                              typeof message.message.id,
                              'string',
                          );
                          assert.strictEqual(
                              typeof message.message.created,
                              'number',
                          );
                      });
                  }
